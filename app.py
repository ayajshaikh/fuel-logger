...
def add_record():
    try:
        ...
        conn = sqlite3.connect('petrol_records.db')
        c = conn.cursor()
        c.execute('INSERT INTO records (date, liters, cost, odometer, rate, station_id,vehicle_id,fuel_type_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
...
def view_records():
    conn = sqlite3.connect('petrol_records.db')
    c = conn.cursor()
    c.execute('''SELECT records.id, records.date, ref_fuel.name, records.liters, records.cost, records.odometer, records.rate,
...
def edit_record(id):
    ...
    if request.method == 'POST':
        ...
        c.execute('UPDATE records SET date = ?, liters = ?, cost = ?, odometer = ?, rate = ?, station_id = ? , vehicle_id= ?, fuel_type_id=? WHERE id = ?',
...
def analytics():
    conn = sqlite3.connect('petrol_records.db')
    c = conn.cursor()
    c.execute('''SELECT records.date, records.liters, records.cost, records.odometer, records.rate,
                        stations.name, vehicle.registration
                 FROM records
                 JOIN vehicle ON records.vehicle_id = vehicle.id
                 JOIN stations ON records.station_id = stations.id ORDER BY records.date ASC''')
    data = c.fetchall()
    conn.close()
